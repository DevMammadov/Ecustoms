...
import { ISearchResponse, IRemoveDoc, IUpdateDoc } from "views/xif-docs/types";
...
export default class XifDocsApi {
  ...
  static sendFilterForm = (data: ISearchResponse) => {
    return defaultRequest.get(`/xif-docs/search`, { params: data }).then(({ data }) => data);
  };
  ...
  static updateDoc = (payload: IUpdateDoc) => {
    return defaultRequest
      .put(`/xif-docs/${payload.id}`, payload.data, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      })
